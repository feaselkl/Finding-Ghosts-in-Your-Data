...
def detect_multi_timeseries(
    ...
    if (num_data_points / num_series < 15):
        ...
        (df_out, diag_outliers) = determine_outliers(df_tested, tests_run, sensitivity_score, max_fraction_anomalies)
...
def run_tests(df):
    ...
    diagnostics = {
        ...
        "Total number of time series segments": num_segments,
        "Number of segments per time series": num_subsegments
    }
...
def check_diffstd(segments, segment_means, num_segments, num_subsegments):
    ...
    for i in range(num_segments):
        for j in range(num_subsegments):
            ...
            segments[i][j]['anomaly_score'] = diffstd(segments[i][j]['value'], segment_means[j])
    return segments
...
def determine_outliers(
    ...
    # Therefore, to reduce the number of results, we'll start with 1.5 * mean of anomaly scores as a max distance score.
    ...
    series = [y for x, y in df.groupby("series_key", as_index=False)]
    segment_scores = [s.drop_duplicates(subset = ['segment_number', 'anomaly_score'], keep='last') for s in series]
    means = [s["anomaly_score"].mean() for s in segment_scores]
    diagnostics = { "Series means": means }

    sensitivity_thresholds = [m + (1.5 - (sensitivity_score / 100.0)) * m for m in means]
    diagnostics["Sensitivity thresholds"] = sensitivity_thresholds

    # Get the 100-Nth percentile of anomaly score.
    # Ex:  if max_fraction_anomalies = 0.1, get the
    # 90th percentile anomaly score.
    max_fraction_anomaly_scores = [np.quantile(df['anomaly_score'], 1.0 - max_fraction_anomalies) for df in segment_scores]
    ...
    sensitivity_thresholds = [max(st, mfa) for (st, mfa) in zip(sensitivity_thresholds, max_fraction_anomaly_scores)]
